...
- **Spacing.** Stamps are placed at a configurable percentage of brush size along the stroke path. The path between samples is densified using **Catmull-Rom tangent-based segment shaping** — a one-sample lookahead is used to derive C1-continuous tangents at each raw sample, and each segment is subdivided by chord length, placing stamps at regular canvas-pixel intervals. This is not a full Catmull-Rom spline (which would defer rendering until both forward neighbors arrive) but a per-segment evaluation that uses the same tangent math. See [`design/STROKES.md`](../../design/STROKES.md) for the precise implementation.
