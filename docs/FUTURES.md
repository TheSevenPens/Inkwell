...
- ~~**Vector eraser.** Hit-test strokes against the eraser path and either remove whole strokes or split them at intersection points.~~ **Shipped** with three modes (Edit → Vector Eraser Mode): Whole Stroke, Touched Region, To Intersection. V1 caveats and V2 follow-ups:
  - **Touched Region cuts at sample boundaries, not sub-pixel disc edges.** A sparse polyline can leave a stub on either side of the erased gap. V2: clip each segment analytically against the disc and emit synthetic entry / exit samples (with linearly-interpolated pressure) so cuts are pixel-precise regardless of sample density.
  - **To Intersection uses strict CCW transversal**: exactly-collinear or exactly-touching segments don't count as crossings. Pure quantization makes this rare in practice; if it ever bites, the fix is a tolerance-aware predicate.
  - **To Intersection self-intersection only counts non-adjacent segments**: segments sharing a sample (i and i±1) are excluded so the trivial "shared-vertex meeting" doesn't always stop the walk. Loops where the stroke crosses itself with at least one segment in between work as expected.
