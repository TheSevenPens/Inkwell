...
- ~~**Vector eraser.** Hit-test strokes against the eraser path and either remove whole strokes or split them at intersection points.~~ **Shipped** with three modes (Edit → Vector Eraser Mode): Whole Stroke, Touched Region, To Intersection. Sample-boundary cuts in Touched Region and strict-crossing detection in To Intersection are noted V1 caveats; sub-pixel disc-edge clipping and a spatial index for performance are V2 follow-ups.
