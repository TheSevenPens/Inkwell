...
- **No 16-bit precision.** Architecture decision 6 commits to 16-bit per channel for the working space; today we ship `.rgba8Unorm` (8 bpp) everywhere. Visible posterization in deep stacks. Promoting to 16-bit requires: (1) changing tile textures from `.rgba8Unorm` to `.rgba16Unorm` or `.rgba16Float`; (2) updating stamp, ribbon, and mask shaders to read/write 16-bit; (3) doubling undo-snapshot memory budgets (per-tile snapshot grows from 256 KB to 512 KB for bitmap layers); (4) bumping the `tiles.bin` format version since tile byte layout changes; (5) updating the drawable pixel format to carry the precision to screen (see `Known gaps` in [`COMPOSITOR.md`](COMPOSITOR.md)).
